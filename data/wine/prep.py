...
def prep_data(binned = False):
  ...
  s = requests.get("https://archive.ics.uci.edu/ml/machine-learning-databases/wine-quality/winequality-white.csv").content
  dta_white = pd.read_csv(io.StringIO(s.decode('utf-8')),
              index_col = False,
              sep = ';',
              skipinitialspace = True)
  dta_white.insert(0, 'type', 'white')
  
  s = requests.get("https://archive.ics.uci.edu/ml/machine-learning-databases/wine-quality/winequality-red.csv").content
  dta_red = pd.read_csv(io.StringIO(s.decode('utf-8')),
              index_col = False,
              sep = ';',
              skipinitialspace = True)
  ...
  bins = [0, 4, 5, 6, 10]
  labels = [1,2,3,4]
  dta['label'] = pd.cut(dta['quality'], bins = bins, labels = labels)

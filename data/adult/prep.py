...
def prep_data(binned = False):
  ...
  s = requests.get(urls[0]).content
  dta_train = pd.read_csv(io.StringIO(s.decode('utf-8')),
                names = colnames,
                na_values=['?'],
                index_col = False,
                skipinitialspace = True)
  
  s = requests.get(urls[1]).content
  dta_test = pd.read_csv(io.StringIO(s.decode('utf-8')),
                names = colnames,
                na_values=['?'],
                index_col = False,
                skipinitialspace = True)
